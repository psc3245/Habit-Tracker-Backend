...
export async function updateSettings(id, updates) {
    const old = await pool.query('SELECT * FROM settings WHERE user_id = $1', id);
    const existing = old.rows[0];
    if (!existing) return null;

  const display_mode = updates.display_mode ?? existing.display_mode;
  const left_default_page = updates.left_default_page ?? existing.left_default_page;
  const right_default_page = updates.right_default_page ?? existing.right_default_page;
  const default_habit_type = updates.default_habit_type ?? existing.default_habit_type;
  ...
  const result = await pool.query(
    ...
    [display_mode, left_default_page, right_default_page, default_habit_type, id]
  )
